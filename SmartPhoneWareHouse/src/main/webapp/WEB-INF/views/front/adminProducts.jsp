...
<div  id="footerSection">
    ...
    $(function() {
        ...
        var normProduct =function(){
        	 $.ajax({
             	  type: "POST",
             	  url: '<c:url value="/getproductsbySellerId/"/>'+${seller.id}
             	}).done(function(e){
             		 		obj = JSON.parse(e);
             				if(obj.statusCode!="200"){
             					...
             					for(var d in obj.obj){
             						...
             						+"  <a href=\"adminEditProduct/"+obj.obj[d].id+"\"	 class=\"btn btn-large\">Edit</a>"
             						+"  <a  href=\"javascript:onclick=deleteProduct("+obj.obj[d].id+")\"  class=\"btn btn-large\">Delete</a>"
             						+"	</form>"
             						+"</div>"
             						+"</div>"
             						+"<hr class=\"soft\"/>";
    ...
    normProduct();
      ...
      function deleteProduct(id){
        	$.ajax({
            	  type: "GET",
            	  url:"<c:url value="/deleteProductByid/"/>"+id,
